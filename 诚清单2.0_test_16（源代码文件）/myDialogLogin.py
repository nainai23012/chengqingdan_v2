...
from ui_QWDialogLogin import Ui_Dialog
from pickle import dump, load
from myDialogRegister import QmyDialogRegister  # 注册

class QmyDialogLogin(QDialog):
    ...
    def on_pushButton_2_clicked(self):
        print('新用户注册')
        if (self.dlgregObj == None):  # 未创建对话框
            self.dlgregObj = QmyDialogRegister(self)
            self.dlgregObj.setDict()
        ret = self.dlgregObj.exec()  # 以模态方式运行对话框
        # ret = self.dlgInforObj.show()  # 以非模态方式运行对话框
        if (ret == QDialog.Accepted):
            result = self.dlgregObj.getDict()
