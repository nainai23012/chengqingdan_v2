...
from PyQt5.QtWidgets import QApplication, QDialog, QAbstractItemView, QHeaderView, QDialog, QFileDialog
...
from ui_QWDialogImportReinforcementQuantities import Ui_Dialog
from myDelegates import QmyFloatSpinDelegate, QmyComboBoxDelegate  # 定义代理模块


class QmyDialogImportReinforcementQuantities(QDialog):
    def __init__(self, rowCount=3, colCount=5, parent=None):
        ...
        self.ui.btnOK.setEnabled(False)  # 确定按钮 不可用
    ...
    def __initmatching2(self):
        ...
        self.UnitOfMeasurement.setItems(comboboxlist, False)  # 下拉列表添加 条目 不可编辑
    ...
    def on_ListLoad_clicked(self):
        ...
        if filename:
            ...
            with xlrd.open_workbook(filename) as f:
                ...
                self.ui.ListCbx.clear()  # 先清空下拉列表
                self.ui.ListCbx.addItems(sheetsName)
            self.ui.ListLoad.setEnabled(False)
                # print(sheetsName)
...
if __name__ == "__main__":  # 用于当前窗体测试
    ...
    form = QmyDialogImportReinforcementQuantities()  # 创建窗体
    form.show()
    sys.exit(app.exec_())
